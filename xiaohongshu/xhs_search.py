...
import urllib3
import traceback
from CralwerSet.xiaohongshu import xiaohsClass as x

urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)


def get_Xsign(keyword, page):
    data = "/fe_api/burdock/weixin/v2/search/notes?keyword=%s&sortBy=general&page=%s&pageSize=20&needGifCover=trueWSUDD" % (
...
def requestUserDetail(id, xhs):
    url = xhs.userApi%(id)
    result = xhs.getData(url)
    return result

def requestUserList(id,page,xhs):
    url = xhs.searchNoteApiOfUser % (id, page)
    result = xhs.getData(url)
    return result
            ...
            while True:
                ...
                for key in keys:
                    ...
                    if tag == 'note':
                        ...
                        text = requestUserDetail(keyword, xhs)
                    elif tag == 'userNoteList':
                        text = requestUserList(keyword, page, xhs)
                    else:
                        text = json.dumps({
                            'code': 1,
                            'success': False,
